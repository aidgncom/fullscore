...
class Rhythm {
	...
	click(el) { // Click action and cookie refresh
		...
		const current = field?.[0], previous = prevField?.[0];
		current && current >= '1' && (!waf || current > waf) && current !== previous && (document.cookie = 'waf=' + current + '; Path=/', location.reload()); // Update movement security field [0] (OXXXXXXXXX)
