 ...
 * 
 * A client-side engine that leverages users' browsers as auxiliary databases.
 * Unlike traditional approaches which send, store, and process data on servers,
 * RHYTHM embeds functionality directly in the browser. As a result, real-time
 * analysis and immediate response occur at the browser and Edge layers without
 * reaching the origin server. With BEAT at its core, this architecture reduces
 * data transfer, server operation, and processing costs.
 */
	...
	constructor() { // RHYTHM engine start
		...
		document.addEventListener('scroll', () => { // BEAT Scroll position tracking addon (default: false)
			...
			clearTimeout(this.s), this.s = setTimeout(() => {
				if (RHYTHM.ADD.SCR && this.hasBeat) this.beat.time(), this.beat.notes.push(BEAT.TOK.P + Math.round(window.scrollY)); // Record final scroll position
