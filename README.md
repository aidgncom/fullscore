...
BEAT expresses user actions like a performance, letting them flow into rhythm.
...
**Cross-Tab (___N) - Linking multiple BEAT sequences**
- ___1, ___2, ___3: Continuations across browser tabs within the same session
- Browsers as auxiliary databases record parallel sequences immediately and merge them into one journey at the end
- Extends to app instances, device clusters, or any parallel sequences in other domains
...
BEAT uses only RFC 6265 characters that pass smoothly through Edge and other security layers. No separate encoding or decoding is required, and because it's light enough to reside in the browser's native storage, it allows real-time analysis without delay in both browsers and Edge.
...
In the Edge, BEAT sequences can be serialized as NDJSON for clean one-line archiving in BigQuery, while also improving AI processing efficiency. Across Web, Mobile, IoT, Game, Finance and other event-driven systems, BEAT's semantically complete structure allows fast merging and easy compatibility with their respective formats.

BEAT is not meant to replace formats like JSON, but has the potential to move forward alongside them in a complementary way.
...
Full Score is a lightweight library with framework-level orchestration. Simply place it directly in your website's footer, where it will have minimal impact on loading times. It's designed for easy customization. Please refer to the code comments for details.

While real-time analytics and security layers based on event sequences can be implemented directly on the client side, deploying to Edge maximizes Full Score's capabilities with features like WAF blocking, personalized functionality, AI analysis, and log pushing to cloud storage. **Resonator** is the official reference implementation for BEAT interpretation, provided as a separate project under the SSPL-1.0 license.
...
// Basic (2.75KB gzip): BEAT + RHYTHM

<script src="https://cdn.jsdelivr.net/gh/aidgncom/fullscore@main/fullscore.standard.min.js"></script>
// Standard (3.20KB gzip): TEMPO + BEAT + RHYTHM

<script src="https://cdn.jsdelivr.net/gh/aidgncom/fullscore@main/fullscore.extended.min.js"></script>
// Extended (3.37KB gzip): TEMPO + BEAT + RHYTHM(+ADDON)
```

The most recommended version is the Basic version. This version includes only BEAT (core) and RHYTHM (engine), without TEMPO (auxiliary module). It operates without issues on most sites.
...
RHYTHM.KEY = 8;						// Session key length (default: 8 chars)
